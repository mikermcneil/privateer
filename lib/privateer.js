...
var ALL_ABSTRACT_DEFS_BY_IDENTITY = [
  ...
  var abstractDef = require('./abstract-interface-for-exchanges/methods/'+_.snakeCase(identity).toUpperCase());
  ...
  var implementations = {
    ...
    createEachOrder: async function(inputs, exits) {
      ...
      for (let order of inputs.orders) {
        ...
        let unitPriceLimitInQuotePerBase = (
          marketAction === 'sell'?
          // Minimum acceptable ask to make:
          1.001//todo (obtain a reasonable unit price)
          :
          // Maximum acceptable bid to make:
          0.0005//todo (obtain a reasonable unit price)
        );
    ...
    getActiveOrders: async function(inputs, exits) {
      ...
      activeOrders = activeOrders.map((ccxtOrder)=>{
        ...
        console.log('*',ccxtOrder);

        var op;
        var amtBaseToSubtract;
        if (ccxtOrder.side === 'sell') {
          op = currencies.join('»');
          amtBaseToSubtract = String(ccxtOrder.amount);
        } else if (ccxtOrder.side === 'buy') {
          op = _.clone(currencies).reverse().join('»');
          amtBaseToSubtract = String(ccxtOrder.amount*ccxtOrder.price);// TODO: safer math
        } else {
 ...
 * .getUsdEquivalent()
 *
 * Get the USD equivalent of the specified currency and amount.
...
Object.defineProperty(module.exports, 'getUsdEquivalent', {
  enumerable: false,
  configurable: false,
  writable: false,
  value: machine.buildWithCustomUsage({
    arginStyle: 'serial',
    def: {
      friendlyName: 'Get USD equivalent',
      inputs: {
      ...
      fn: async function (inputs, exits){
        ...
        var currencyInfo = _.find(coinMarketCapData, { symbol: currency });
        if (!currencyInfo) {
          throw new Error(`Unrecognized currency (${currency})`);
        }
        var estimatedUsdPrice = currencyInfo.price_usd;

        return exits.success(_.round(estimatedUsdPrice * amount),2);
      }
    }
  })
});
