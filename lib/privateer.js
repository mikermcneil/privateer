...
module.exports = function privateer(exchange){
  ...
  var implementations = {
    ...
    getExchangeRates: async function(inputs, exits) {
      ...
      var MAX_REQUESTS_BEFORE_BREATHER = 10;
      ...
      var approximateUsdEquivs = { USD: 1 };
      {
        for (let currency of currencies) {
          ...
          switch (currency) {
            ...
            case 'BCU': coinMarketCapCode = 'BTU'; break;
            case 'DAT': coinMarketCapCode = 'DATA'; break;
            case 'NBT': coinMarketCapCode = 'USNBT'; break;
            case 'STR': coinMarketCapCode = 'XLM'; break;
            case 'Bitmark': coinMarketCapCode = 'BTM'; break;

            // As of ccxt 1.9.346, these are the currencies that we couldn't find an equivalent code for:
            // EUR
            // GBP
            // RRT
          ...
          if (!currencyInfo){
            console.log('Saw a currency that is not in coinmarketcap!  It is `'+currency+'`');
          }
        }//∞
      }//∫
