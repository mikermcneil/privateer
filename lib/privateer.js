  ...
  var implementations = {
    ...
    getActiveOrders: async function(inputs, exits) {
      ...
      activeOrders = activeOrders.map((ccxtOrder)=>{
        ...
        // - - - - - - - - - - - - - - - - - - - - -
        // TODO: probably also return:
        //  • whether it's been partially filled
        //  • if so, how much much was filled
        //  • and probably also how much remains
        // - - - - - - - - - - - - - - - - - - - - -
        return {
          id: ccxtOrder.id,
          operation: op,
    ...
    getExchangeRates: async function(inputs, exits) {
      ...
      var currencies = ['eth', 'btc', 'usd', 'ltc'];
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // ^^
      // TODO: allow this set of relevant currencies slugs to be passed in
      // (to allow for filtering to speed this up)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

      var ccxtCurrencies = currencies.map(slug => slug.toUpperCase());
      var markets = await ccxtExchange.fetchMarkets();
      var relevantMarkets = _.filter(markets, market => _.contains(ccxtCurrencies, market.base) && _.contains(ccxtCurrencies, market.quote));
      ...
      {// Now for each currency pair, catalog the current market price (in both directions).
        for (let market of relevantMarkets){
          ...
          rates[market.base][market.quote] = ticker.bid;

          rates[market.quote] = rates[market.quote]||{};
          rates[market.quote][market.base] = ticker.ask;
        }//∞
      ...
      // console.log(`${require('util').inspect(rates,{depth:null})}`);
    ...
    getHoldings: async function(inputs, exits) {
      ...
      var freeHoldings = _.reduce(sortedCurrenciesHeld, (freeHoldings, ccxtCurrency)=>{
        var slug = ccxtCurrency.toLowerCase();
        var amount = ccxtBalanceReport.free[ccxtCurrency];
        if (amount !== 0) {
          freeHoldings[slug] = String(ccxtBalanceReport.free[ccxtCurrency]);
        }
        return freeHoldings;
      }, {});
