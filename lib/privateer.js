...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// Info on which API credentials you need for 5 major exchanges, and the
// URL where you can obtain that information:
//
// Bitfinex
...
module.exports = function privateer(exchange){
  ...
  var implementations = {
    ...
    getUsdEquivalents: async function(inputs, exits) {
      ...
      {// Obtain a unique list of currencies traded on our exchange.
        ...
        // > (no need to use credentials-- this one is part of the public API)
        let ccxtExchange = new ccxt[exchangeInfo.ccxtId]();
        await ccxtExchange.fetchMarkets();
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Note: As of Nov 2017, running `ccxtExchange.currencies` is equivalent to the following code.
        // However, I'm leaving it here, commented-out, just in case this changes in the future (some
        // related bits in ccxt seem to be in flux at the moment...)
        // ```
        // let relevantMarkets = await ccxtExchange.fetchMarkets();
        // console.log(_.union(_.pluck(_.values(relevantMarkets), 'base'), _.pluck(_.values(relevantMarkets), 'quote')));
        // console.log('vs.');
        // console.log(ccxtExchange.currencies);
        // ```
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        currencies = ccxtExchange.currencies;
      }//∫
      ...
      // on this exchange and filling in the coinmarketcap data.
      var approximateUsdEquivs = _.reduce(currencies, (equivs, currency)=>{
        let currencyInfo = _.find(coinMarketCapData, { symbol: currency });
        if (!currencyInfo) {
          throw new Error(`Unrecognized currency (${currency}) is not listed in coinmarketcap.`);
        }
        equivs[currency] = currencyInfo.price_usd;
        return equivs;
      }, {});//∞
  ...
  Object.defineProperty(pkg, '_ccxtExchange', {
    enumerable: false,
    configurable: false,
    writable: false,
