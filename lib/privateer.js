...
module.exports = function privateer(exchange){
  ...
  var implementations = {
    ...
    getHoldings: async function(inputs, exits) {
      ...
      // fetchBalance()
      // ```
      let ccxtBalanceReport = await ccxtExchange.fetchBalance();
      // e.g.
      ...
      // Sort the dictionary of available holdings (`.free`)
      // > (Note that we're still dealing with numbers here, not strings yet)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // TODO: instead of sorting by amount held, use some other universal approach.
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      let biggest = 0;
      let currenciesHeldInDescendingOrder = [];
      for (let ccxtCurrency in ccxtBalanceReport.free) {
        let amount = ccxtBalanceReport.free[ccxtCurrency];
        if (amount > biggest) {
          currenciesHeldInDescendingOrder.unshift(ccxtCurrency);
          biggest = amount;
        }
        else {
          currenciesHeldInDescendingOrder.push(ccxtCurrency);
        }
      }//∞
      ...
      let freeHoldings = _.reduce(currenciesHeldInDescendingOrder, (freeHoldings, ccxtCurrency)=>{
        let slug = ccxtCurrency.toLowerCase();
        let amount = ccxtBalanceReport.free[ccxtCurrency];
        if (amount !== 0) {
