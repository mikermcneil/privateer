...
`,
      ...
      custom: (orders)=>{
        var RX_IS_LOWERCASE_CURRENCY_SYMBOL = /[a-z0-9]+/;
        for (let order of orders) {
          let pieces = order.operation.split(/[\»\>]+/);
          if (pieces.length !== 2) {
            return false;
          }
          if (!pieces[0].match(RX_IS_LOWERCASE_CURRENCY_SYMBOL) || !pieces[1].match(RX_IS_LOWERCASE_CURRENCY_SYMBOL)) {
            return false;
          }
        }//∞
        return true;
